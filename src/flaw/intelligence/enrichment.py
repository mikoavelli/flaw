...
def enrich(
    ...
    *,
    ...
    Args:
        ...
        skip_enrich: If True, skip EPSS/KEV lookup (CVSS-only mode).
    ...
    if not skip_enrich:
        epss.ensure_fresh(conn, cache_dir)
        kev.ensure_fresh(conn, cache_dir)
    ...
    for v in vulnerabilities:
        enriched.append(
            EnrichedVulnerability(
                ...
                has_exploit=v.cve_id in kev_set,  # KEV implies public exploit
                risk_score=0.0,  # Scored in next step
            )
        )
